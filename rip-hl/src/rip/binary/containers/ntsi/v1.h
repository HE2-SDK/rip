...
namespace rip::binary::containers::ntsi::v1 {
    struct FileHeader {
        ucsl::magic_t<4> magic; // NTSI
        unsigned int version;
        unsigned int unk0;
        unsigned int nameSize;
        unsigned int defaultMipSize;
        unsigned int defaultMipIdx;
    };
}

namespace rip::util {
    template<> inline void byteswap_deep(rip::binary::containers::ntsi::v1::FileHeader& value) noexcept {
...
namespace rip::binary::containers::ntsi::v1 {
    ...
    class NeedleTextureStreamingInfoReader {
    private:
    ...
    public:
        ...
        NeedleTextureStreamingInfoReader(std::istream& stream_) : raw_stream{ stream_ }, stream{ raw_stream, endianness } {
            stream.read(header);
            stream.read_string(packageName);
        }
