...
namespace rip::util {
	...
	template<> inline void byteswap_deep(rip::binary::containers::swif::v1::SRS_END_CHUNK_HEADER& value) noexcept {
	}
}
...
namespace rip::binary::containers::swif::v1 {
	...
	class SWIFReader {
		...
		SRS_CHUNK_HEADER fileChunkHeader;
		SRS_BINARY_FILE_HEADER_CHUNK_HEADER fileHeader;

	public:
		SWIFReader(BinaryInputStreamType& parent_stream) : stream{ parent_stream.get_raw_stream(), parent_stream.endianness, 0 } {
			stream.read(fileChunkHeader);
			stream.read(fileHeader);
			stream.seekg(fileHeader.chunksStart);
		}

		template<typename F>
		void for_each_chunk(F f) {
			for (size_t i = 0; i < fileHeader.chunkCount; i++) {
				SRS_CHUNK_HEADER chunkHeader;

				stream.read(chunkHeader);

				size_t dataStart = stream.tellg();

				f(chunkHeader.magic, stream);

				stream.seekg(dataStart + chunkHeader.chunkSize);
			}
		}
	};
	...
	public:
		class swif_ostream : public BinaryOutputStreamType {
		protected:
			SWIFWriter& writer;

		public:
			swif_ostream(SWIFWriter& writer, BinaryOutputStreamType& parent_stream, size_t offset) : writer{ writer }, BinaryOutputStreamType{ parent_stream.get_raw_stream(), offset } {
			}
			...
			void write(const offset_t<T>& obj) {
				if (obj.has_value())
					writer.addressLocations.emplace_back(static_cast<unsigned int>(this->tellp()));
		...
		template<typename ChunkHeader>
		class chunk_ostream : public swif_ostream {
		protected:
			SWIFWriter& writer;
		...
		public:
			chunk_ostream(const ucsl::magic_t<4>& magic, SWIFWriter& writer, const ChunkHeader& header) : magic{ magic }, writer{ writer }, swif_ostream{ writer, writer.stream, 0 } {
				chunkStart = this->tellp();
				this->write(SRS_CHUNK_HEADER{});
				dataStart = this->tellp();
				this->write(header);
			...
			void finish() {
				...
				size_t dataEnd = this->tellp();

				this->seekp(chunkStart);
				this->write(SRS_CHUNK_HEADER{ magic, static_cast<unsigned int>(dataEnd - dataStart) });
				this->seekp(dataEnd);
			}
		};
	...
	private:
		...
		void writeFileHeaderChunk(const SRS_BINARY_FILE_HEADER_CHUNK_HEADER& header) {
			chunk_ostream<SRS_BINARY_FILE_HEADER_CHUNK_HEADER> chunkStream{ "SWIF", *this, header };
		}

		void writeAddressResolutionChunk() {
			chunk_ostream<SRS_ADDRESS_RESOLUTION_CHUNK_HEADER> chunkStream{ "SOF0", *this, { static_cast<unsigned int>(addressLocations.size()), 0 } };

			for (unsigned int addressLocation : addressLocations)
				chunkStream.write(addressLocation);
		}

		void writeEndChunk() {
			chunk_ostream<SRS_END_CHUNK_HEADER> chunkStream{ "SEND", *this, {} };
		}

	public:

		SWIFWriter(BinaryOutputStreamType& parent_stream) : stream{ *this, parent_stream, parent_stream.get_raw_stream().tellp() } {
			writeFileHeaderChunk({});
			header.chunksStart = stream.tellp();
		}
		...
		chunk_ostream<SRS_TEXTURELIST_CHUNK_HEADER> add_texture_list_chunk(unsigned int textureListCount) {
			header.chunkCount++;

			return { "SWTL", *this, { 16, textureListCount } };
		}

		chunk_ostream<SRS_PROJECT_CHUNK_HEADER> add_project_chunk() {
			header.chunkCount++;
		...
		void finish() {
			header.addressResolutionHeaderOffset = stream.tellp();

			writeAddressResolutionChunk();
			writeEndChunk();

			header.chunksSize = header.addressResolutionHeaderOffset - header.chunksStart;
			header.revision = 20120705;

			stream.seekp(0);
			writeFileHeaderChunk(header);
		}
	};
