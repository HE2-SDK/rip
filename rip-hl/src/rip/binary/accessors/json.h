...
namespace rip::binary::accessors {
	...
	struct json {
		...
		struct Reference {
			...
			inline Reference(yyjson_doc* doc, yyjson_val* val) : doc{ doc }, val{ val }, kind{ Kind::CUSTOM } {}
			inline Reference(yyjson_doc* doc) : doc{ doc }, val{ yyjson_doc_get_root(doc) }, kind{ Kind::ROOT } {}
			inline Reference(const Reference& parent, const char* key) : doc{ parent.doc }, val{ yyjson_obj_get(parent, key) }, kind{ Kind::PROPERTY }, ref{ .property = { .obj = parent, .key = key } } {}
			inline Reference(const Reference& parent, size_t index, yyjson_val* val = nullptr) : doc{ parent.doc }, val{ val ? val : yyjson_arr_get(parent, index) }, kind{ Kind::INDEX }, ref{ .index = { .arr = parent, .index = index } } {}

			inline operator yyjson_val*() const {
				if (val != nullptr)
					return val;
		...
		public:
			...
			template<typename FieldRefl>
			constexpr const auto operator[](const FieldRefl& field_refl) const {
				auto type = field_refl.get_type(*this);
		...
		template<typename Refl>
		class UnionAccessor : public Accessor<Refl> {
		public:
			using Accessor<Refl>::Accessor;
		...
		public:
			...
			public:
				...
				constexpr bool operator==(const const_iterator& other) const { return idx == other.idx; }
				constexpr bool operator!=(const const_iterator& other) const { return idx != other.idx; }
				constexpr bool operator<(const const_iterator& other) const { return idx < other.idx; }
				constexpr bool operator>(const const_iterator& other) const { return idx > other.idx; }
				constexpr bool operator<=(const const_iterator& other) const { return idx <= other.idx; }
			...
			inline size_t size() const { return yyjson_arr_size(this->reference); }
			inline size_t capacity() const { return yyjson_arr_size(this->reference); }
		};
