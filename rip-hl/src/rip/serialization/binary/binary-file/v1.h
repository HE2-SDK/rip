...
namespace rip::serialization {
	...
	struct binary<models::BinaryFileV1<Refl, AllocatorSystem>> {
		inline static void save(auto& stream, const models::BinaryFileV1<Refl, AllocatorSystem>& model) {
			...
			rip::binary::containers::binary_file::v1::BinaryFileWriter<typename std::decay_t<decltype(stream)>::RawStreamType, typename std::decay_t<decltype(stream)>::AddrType, std::decay_t<decltype(stream)>::endianness> writer{stream.get_raw_stream()};
		...
		inline static models::BinaryFileV1<Refl, AllocatorSystem> load(auto& stream) {
			// We probably want to revert to BFW taking a binary_ostream but avoiding that refactor atm by unpacking the raw stream here.
			// Maybe endianness should also just not be type level.
